...
{
    private Stack<GameObject> SearchStack            = new Stack<GameObject>();
    private List<GameObject>  AlreadySearchedList   = new List<GameObject>();
    //-----------------------------------------------------------------------------------

    public override void Search()
    {
        Debug.Log("---DFS Search---");
        ...
        SearchStack.Push(Grid.grid[Grid.StartX, Grid.StartY]);
        ...
        {
            GameObject currentObject = SearchStack.Pop();
            if (currentObject != null)
            {
                ...
                if (currentNode != null && currentCell != null)
                {
                    ...
                    for (int i = 0; i < childObjects.Count; i++)
                    {
                        ...
                        if (childNode != null)
                        {
                            if (!IsInList(AlreadySearchedList, childObjects[i]) && !IsInList(SearchStack, childObjects[i]))
                            {
                                childNode.ParentX = currentCell.xPos;
                                childNode.ParentY = currentCell.yPos;
                                SearchStack.Push(childObjects[i]);
                            }
                        }
                    }
                }
    ...
    public override void UpdateVisuals()
    {
        ...
        if (SearchStack.Count > 0)
        {
            foreach (GameObject obj in SearchStack)
            {
